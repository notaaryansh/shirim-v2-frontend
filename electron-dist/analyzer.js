...
export const ENV_VAR_RE = /(?<![A-Z0-9_])([A-Z][A-Z0-9_]{3,})(?=\s*[:=]|\s*\?=|\s*\)|\s*\.|\s*["'])/g;
...
// -------------------- helpers --------------------
export function readFiles(workdir, tree) {
    const result = {};
...
export function scanEnvVars(files, treeSample) {
    const seen = new Map();
    ...
    for (const name of ['README.md', 'README.rst', 'README.txt', 'README']) {
        ...
        for (const m of matches) {
            const varName = m[1];
            if (ENV_VAR_BLOCKLIST.has(varName))
                continue;
            if (!seen.has(varName)) {
                seen.set(varName, {
                    name: varName,
...
export function languageDistribution(tree) {
    const extCounts = {};
    let total = 0;
    for (const f of tree) {
...
export function analyze(workdir, adapters) {
