...
export class PythonAdapter {
    ...
    detect(tree, _files) {
        ...
        if (pyCount > 0)
            score += Math.min(pyCount / 20, 0.4);
        if (tree.includes('Pipfile.lock'))
            score += 0.1;
        if (tree.includes('poetry.lock'))
            score += 0.1;
    ...
    parseDeps(workdir, tree, files) {
        ...
        if (tree.includes('Pipfile.lock'))
            parsed.depFiles.push('Pipfile.lock');
        if (tree.includes('poetry.lock'))
            parsed.depFiles.push('poetry.lock');
        ...
        if (reqContent) {
            for (const line of reqContent.split('\n')) {
                ...
                if (depName)
                    parsed.declaredDeps.push(depName);
            }
        }
    ...
    _hasPoetrySection(content) {
        if (!content)
            return false;
        return /\[tool\.poetry\b/.test(content);
    }
    _parsePyprojectDeps(content, parsed) {
    ...
    _extractDepsFromArray(arrayContent, parsed) {
        const depLineRe = /["']([^"']+)["']/g;
        let match;
