...
export class RustAdapter {
    ...
    detect(tree, _files) {
        let score = 0;
        if (tree.includes('Cargo.toml'))
            score += 0.5;
        if (tree.includes('Cargo.lock'))
            score += 0.1;
        const rsCount = tree.filter(f => f.endsWith('.rs')).length;
        if (rsCount > 0)
            score += Math.min(rsCount / 20, 0.4);
    ...
    parseDeps(workdir, tree, files) {
        ...
        if (tree.includes('Cargo.toml'))
            parsed.depFiles.push('Cargo.toml');
        if (tree.includes('Cargo.lock'))
            parsed.depFiles.push('Cargo.lock');
    ...
    _parseCargoToml(content, parsed) {
    ...
    _extractDepsSection(content, sectionRe, parsed) {
