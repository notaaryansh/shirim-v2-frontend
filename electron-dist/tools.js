...
export function createToolContext(workdir, sandboxEnv = {}, pathPrepend = [], secrets = {}) {
...
function getCloseMatches(needle, candidates, n = 3, cutoff = 0.4) {
    const scored = [];
...
export function bash(ctx, command, timeout, phase) {
    const effectiveTimeout = Math.min(timeout ?? ctx.defaultTimeout, ctx.maxTimeout);
    return new Promise((resolve) => {
        const env = { ...process.env, ...ctx.sandboxEnv };
...
export function readFile(ctx, filePath) {
    const full = safePath(ctx.workdir, filePath);
    if (full === null)
        return { ok: false, error: `path escapes workdir: ${filePath}` };
...
export function listFiles(ctx, dirPath = '.') {
    const full = safePath(ctx.workdir, dirPath);
    if (full === null)
        return { ok: false, error: `path escapes workdir: ${dirPath}` };
    if (!fs.existsSync(full))
...
export function editFile(ctx, filePath, oldString, newString) {
    const full = safePath(ctx.workdir, filePath);
    if (full === null)
        return { ok: false, error: `path escapes workdir: ${filePath}` };
...
export function createFile(ctx, filePath, content) {
    const full = safePath(ctx.workdir, filePath);
    if (full === null)
        return { ok: false, error: `path escapes workdir: ${filePath}` };
    if (fs.existsSync(full)) {
...
export const TOOL_SCHEMAS = [
    ...
    },
    {
        ...
        function: {
            name: 'edit_file',
            description: 'Make a targeted, diff-only edit: replace a unique substring in a file. ' +
    ...
    },
    {
        ...
        function: {
            name: 'create_file',
            description: 'Create a new file with the given content. Fails if the file already ' +
    ...
    },
    {
        ...
        function: {
            name: 'report_success',
            description: 'Terminal tool: call this once you\'ve confirmed the repo installs and ' +
...
export const TOOL_IMPLS = {
    bash: (ctx, command, timeout, phase) => bash(ctx, command, timeout, phase),
    read_file: (ctx, filePath) => readFile(ctx, filePath),
    list_files: (ctx, dirPath) => listFiles(ctx, dirPath),
    edit_file: (ctx, filePath, oldStr, newStr) => editFile(ctx, filePath, oldStr, newStr),
    create_file: (ctx, filePath, content) => createFile(ctx, filePath, content),
};
