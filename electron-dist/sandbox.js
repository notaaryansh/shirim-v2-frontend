 ...
 */
export function safePath(workdir, rel) {
    try {
        const full = path.resolve(workdir, rel);
        const workdirAbs = path.resolve(workdir);
        if (full === workdirAbs)
            return full;
        if (!full.startsWith(workdirAbs + path.sep))
            return null;
        return full;
    }
    catch {
 ...
 */
export function walkRepoTree(workdir, maxFiles = 600) {
    const results = [];
    function walk(dir) {
        ...
        for (const entry of entries) {
            ...
            const hasHidden = parts.some((p, i) => i > 0 && p.startsWith('.') &&
 ...
 */
export function cloneRepo(owner, repo, workdir, ref, signal, timeout = CLONE_TIMEOUT_SECONDS) {
...
export function cleanupInstall(installId) {
