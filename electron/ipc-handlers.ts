...
export function registerIpcHandlers(): void {
  ...
  ipcMain.handle('secrets:add', (_e, name: string, value: string) => {
  ...
  ipcMain.handle('secrets:reveal', (_e, name: string) => {
    const value = vault.get(name);
    if (value === null) throw new Error(`Secret "${name}" not found`);
    return { name, value };
  });

  ipcMain.handle('secrets:check', (_e, names: string[]) => {
    return { status: vault.check(names) };
  });

  // ===== Install size =======================================================
  ipcMain.handle('install:get-size', (_e, installId: string) => {
    try {
  ...
  // ===== Install (agent loop) ===============================================
  ipcMain.handle('install:start', async (_e, owner: string, repo: string, ref?: string, authToken?: string) => {
  ...
  ipcMain.handle('install:cancel', (_e, installId: string) => {
    return cancelInstall(installId);
  });

  ipcMain.handle('install:delete', (_e, installId: string) => {
    deleteInstall(installId);
  });

  ipcMain.handle('install:get-progress', (_e, installId: string) => {
    const run = getInstall(installId);
    if (!run) throw new Error(`Install not found: ${installId}`);
    return computeProgress(run);
  });

  ipcMain.handle('install:refresh-token', (_e, installId: string, token: string) => {
    refreshToken(installId, token);
  });

  // ===== Run (launcher) =====================================================
  ipcMain.handle('run:start', async (_e, installId: string, options?: { command?: string; wait_for_url?: number }) => {
    ...
    if (!command) throw new Error('No run command available');
    ...
    const handle = await launcher.startRun(
      installId, command, installDir, sandboxEnv, pathPrepend,
      vault.load(), options?.wait_for_url ?? 30,
    );
    return launcher.toResponse(handle);
  });

  ipcMain.handle('run:stop', async (_e, installId: string) => {
    const handle = launcher.getRunForInstall(installId);
    if (!handle) throw new Error(`No active run for install: ${installId}`);
    await launcher.stopRun(handle.runId);
    return launcher.toResponse(handle);
  });

  ipcMain.handle('run:get-state', (_e, installId: string) => {
    const handle = launcher.getRunForInstall(installId);
    if (!handle) throw new Error(`No run found for install: ${installId}`);
    return launcher.toResponse(handle);
  });

  ipcMain.handle('run:get-logs', (_e, installId: string, limit?: number) => {
    const handle = launcher.getRunForInstall(installId);
    if (!handle) return { lines: [] };
    const lines = handle.logTail.slice(-(limit ?? 200)).map((entry) =>
      typeof entry === 'string' ? entry : (entry as Record<string, unknown>).line ?? ''
    );
    return { lines };
  });

  // ===== Edit (edit loop) ===================================================
  ipcMain.handle('edit:send', async (_e, installId: string, message: string, sessionId?: string | null) => {
    let session: editLoop.EditSession | null = null;
    if (sessionId) {
  ...
  ipcMain.handle('edit:get-session', (_e, installId: string) => {
    const session = editLoop.getSessionForInstall(installId);
    if (!session) throw new Error(`No edit session for install: ${installId}`);
    return {
      session_id: session.session_id,
  ...
  ipcMain.handle('edit:undo', (_e, installId: string, turnId: number) => {
    const session = editLoop.getSessionForInstall(installId);
    if (!session) throw new Error(`No edit session for install: ${installId}`);
