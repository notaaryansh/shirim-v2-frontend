/**
 * Prompt templates for the install agent.
 * Port of shirim-v2-backend/app/agent/prompts.py
 */
import type { Language, ParsedDeps } from './adapters/base.js';
...
export const LANGUAGE_APPENDICES: Record<Language, string> = {
  python: `
...
export function buildInitialUserMessage(
  ...
  sandboxNotes: string[],
  secretNames: string[],
  installCmdHint: string | null,
  smokeRunHints: string[],
): string {
  const parts: string[] = [];
  ...
  parts.push('');
  parts.push(
    'Start by reading the README (if any) and key config files, then install ' +
...
export function buildFallbackUserMessage(
  ...
  parts.push('');
  parts.push(
    'Start by listing files, reading any error logs, and trying an ' +
