/**
 * Prompt templates and tool schemas for the edit agent.
 * Port of shirim-v2-backend/app/agent/edit_prompts.py
 */
import type { AppContext } from './edit-context.js';
...
function formatAppContext(ctx: AppContext): string {
  const lines: string[] = [];

  lines.push(`- Project type: ${ctx.projectType}`);
  if (ctx.framework) lines.push(`- Framework: ${ctx.framework}`);
  if (ctx.styling) lines.push(`- Styling: ${ctx.styling}`);
  if (ctx.uiLibrary) lines.push(`- UI library: ${ctx.uiLibrary}`);
...
export function buildEditSystemPrompt(appContext: AppContext): string {
...
export const EDIT_TOOL_SCHEMAS: Record<string, unknown>[] = [
  {
    type: 'function',
    function: {
      name: 'bash',
      description:
        'Run a shell command in the working directory. Use for installing packages, ' +
  ...
  },
  {
    ...
    function: {
      name: 'edit_file',
      description:
        'Make a targeted edit: replace a unique substring in a file. ' +
