...
export const STEP_DEFS: Array<[string, string]> = [
  ['prepare', 'Preparing'],
  ['analyze', 'Analyzing'],
  ['install', 'Installing'],
  ['test', 'Testing'],
 ...
 */
export function activeIndex(status: string, phase: string | null): number {
  if (status === 'cancelled') return _CANCELLED;
  if (status === 'success') return _SUCCESS;
  if (status === 'failure' || status === 'error') return _FAILED;

  // Running — map phase to step index
  if (!phase) return 0; // prepare
 ...
 */
export function inferFailedIndex(run: AgentRunLike): number {
  let lastPhaseIdx = 0;
 ...
 */
export function extractError(run: AgentRunLike): string | null {
  // Check result for error info
  if (run.result) {
    const reason = run.result['reason'] as string | undefined;
    if (reason) return reason;

    const lastError = run.result['last_error'] as string | undefined;
...
export interface ProgressPayload {
  ...
  owner: string;
  repo: string;
  overall: 'pending' | 'running' | 'success' | 'failure' | 'cancelled';
  steps: StepInfo[];
  error: string | null;
  started_at: number;
  finished_at: number;
 ...
 */
export function computeProgress(run: AgentRunLike): ProgressPayload {
  ...
  const steps: StepInfo[] = STEP_DEFS.map(([id, label], i) => {
    let stepStatus: StepInfo['status'];
